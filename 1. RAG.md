...
### 6. Multi-Query RAG Architecture:   
   a) Multi-Query : Different Perspectives  - (Using a prompt template with instruction to generate N number of different versions queries from original user query retrieve relevant documents from a vector database)    
   b) RAG-Fusion - (combines multiple retrieval sources, each specialized in a unique context, to generate a more accurate, contextually rich response)        
